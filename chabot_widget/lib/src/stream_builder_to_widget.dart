...
class StreamBuilderToWidget<T> extends StatelessWidget {
  const StreamBuilderToWidget(
      {Key? key,
      this.initialData,
      this.stream,
      required this.builder,
      this.noHasDataWidget})
      : super(key: key);
...
class StreamBuilderToSliverList<T> extends StatelessWidget {
  const StreamBuilderToSliverList(
      {Key? key, this.stream, required this.builder, this.emptyWidget})
      : super(key: key);

  final Stream<List<T>>? stream;
  final Widget Function(BuildContext context, T data, int index, int size)
      builder;
  ...
  static SliverList emptySliverList() => SliverList(
          delegate: SliverChildBuilderDelegate((context, index) {
        return Container();
      }, childCount: 0));
}
